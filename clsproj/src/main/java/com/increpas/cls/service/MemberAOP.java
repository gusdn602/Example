...
public class MemberAOP {
	 ...
	 * 와일드 카드
	 * 
	 * "execution(접근지정자 반환값타입 경로.함수)" ==> 경로.함수 중 선언된 접근지정자와 일치하고 반환값타입이 같은 함수가 실행될
	 * 떄를 의미한다. .. 0개 이상 1개 이상
	 */
	
	/*
	 * @Pointcut("execution(* com.increpas.cls.controller.member.Member.*Form(..))")
	 * public void loginCk() { System.out.println("### pointcut exec");
	 * 
	...
	"execution(* com.increpas.cls.service.BoardService.boardWriteProc(..))")
	// @Before는 매개변수에 선언된 내용이 실행되기 이전 실행한다는 의미, 반대의 개념은 @After가 있고
	// 둘을 모두 포함하는 @Arraound도 있다.
	public void memberAutho(JoinPoint join) {
		System.out.println("*******memberAutho()");
		/*
		 * JoinPoint 포인트컷에 선언된 함수의 매개변수를 관리할 클래스 JoinPoint 객체에 실행함수의 매개변수를 꺼내는 방법
		 * 
		 * Object[] obj = 변수.getArgs();
		 * 
		 * 따라서 꺼낼때는 인덱스로 꺼내야 하고 강제형변환도 해줘야 한다.
		 */
		// 매개변수 꺼내기
		...
		tmp = tmp.substring(tmp.lastIndexOf("/") + 1, tmp.lastIndexOf(".")); // /clsproj/member/login.cls ==>
																				// login.cls
		...
		if (sid == null) {
			...
			// 이미 로그인 된 상태
			((ModelAndView) obj[1]).setView(new RedirectView("/clsproj/main"));
	...
	public void loginCK1(JoinPoint join) {
		...
		if (sid == null) {
			// 이 경우는 로그인이 안된 상태
			((ModelAndView) obj[1]).setView(new RedirectView("/clsproj/member/login.cls"));
			((ModelAndView) obj[1]).addObject("isLogin", false);
		} else {
			// 이미 로그인 된 상태
			((ModelAndView) obj[1]).addObject("isLogin",true);
